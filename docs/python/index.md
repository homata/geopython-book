...
ソースコード管理
---
プログラミングしたコードのバージョン管理にGit/GitHubを使用することがデファクトスタンダード（事実上の標準）となっています。

gitは、Linuxのカーネル開発の為に作成されたバージョン管理のツールで、変更点の履歴や多人数による開発を用意にする為のツールです。
git以外にもたくさんのバージョン管理ツールはありますが事実上の標準として使われています。
GitHubは、GitHub社が提供しているフリーミアムサービスで、gitを使いやすくする為の機能が追加されがものです。

※フリーミアムとは: 基本的なサービスや製品は無料で提供し、さらに高度な機能や特別な機能については料金を課金する仕組みのビジネスモデル
